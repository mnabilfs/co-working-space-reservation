...
export default function UserDashboard() {
  ...
  const fetchSpaces = async () => {
    try {
      const res = await fetch('http://localhost:5000/api/spaces', { cache: 'no-store' });
  ...
  const fetchMyReservations = async () => {
    try {
      const res = await fetch(`http://localhost:5000/api/reservations/user/${user.id}`, { cache: 'no-store' });
      const data = await res.json();
  ...
  const fetchBookedSlots = async (spaceId, date) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/reservations/booked-slots?spaceId=${spaceId}&date=${date}&_t=${Date.now()}`, { cache: 'no-store' });
      const data = await res.json();
