...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    try {
      const [spacesRes, resRes, usersRes] = await Promise.all([
        fetch(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/spaces`, { cache: 'no-store' }),
        fetch(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/reservations`, { cache: 'no-store' }),
        fetch(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/users`, { cache: 'no-store' })
      ]);
  ...
  const toggleSpaceStatus = async (space) => {
    ...
    try {
      await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/spaces/${space.id}`, {
        method: 'PUT',
  ...
  const handleDeleteSpace = async (id) => {
    if(window.confirm('Apakah Anda yakin ingin menghapus ruangan ini?')) {
      try {
        await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/spaces/${id}`, { method: 'DELETE' });
        fetchData();
      } catch (err) { console.error(err); }
    }
  };
  ...
  const handleSaveSpace = async (e) => {
    ...
    try {
      if (editingSpace) {
        await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/spaces/${editingSpace.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(formData)
        });
      } else {
        await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/spaces`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(formData)
        });
  ...
  const updateReservationStatus = async (res, newStatus) => {
    try {
      await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/reservations/${res.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
